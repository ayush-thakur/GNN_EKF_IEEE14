...
def generate_dataset(datasetType, data_dir, should_connect_node_to_second_neighbours=True):
    ...
    """
    print("reading from ekf se files started")
    numGraphs, numVariableNodes, numMeasurements, estimate_rows, jacobian_rows, measurement_rows, variance_rows, covariance_rows = read_from_ekf_se_files(
        datasetType, data_dir)
    print("reading from ekf se files done")
...
def read_from_ekf_se_files(datasetType, data_dir):

    if datasetType == "test":
        print("..........................talking of test")
    elif datasetType == "validation":
        print("..........................talking of validation")
    else:
        print("..........................talking of training")

    if datasetType == "test":
        path = str(data_dir) + "/Test_estimates.csv"
    elif datasetType == "validation":
        path = str(data_dir) + "/Validation_estimates.csv"
    else:
        path = str(data_dir) + "/Training_estimates.csv"
    with open(path, 'r') as read_obj:
        csv_reader = reader(read_obj)
        estimate_rows = list(csv_reader)

    if datasetType == "test":
        path = str(data_dir) + "/Test_HJacobian.csv"
    elif datasetType == "validation":
        path = str(data_dir) + "/Validation_HJacobian.csv"
    else:
        path = str(data_dir) + "/Training_HJacobian.csv"
    with open(path, 'r') as read_obj:
        csv_reader = reader(read_obj)
        jacobian_rows = []
    ...
    if datasetType == "test":
        path = str(data_dir) + "/Test_measurements.csv"
    elif datasetType == "validation":
        path = str(data_dir) + "/Validation_measurements.csv"
    else:
        path = str(data_dir) + "/Training_measurements.csv"
    with open(path, 'r') as read_obj:
        csv_reader = reader(read_obj)
        measurement_rows = list(csv_reader)

    if datasetType == "test":
        path = str(data_dir) + "/Test_variances.csv"
    elif datasetType == "validation":
        path = str(data_dir) + "/Validation_variances.csv"
    else:
        path = str(data_dir) + "/Training_variances.csv"
    with open(path, 'r') as read_obj:
        csv_reader = reader(read_obj)
        variance_rows = list(csv_reader)

    if datasetType == "test":
        path = str(data_dir) + "/Test_covariances.csv"
    elif datasetType == "validation":
        path = str(data_dir) + "/Validation_covariances.csv"
    else:
        path = str(data_dir) + "/Training_covariances.csv"
    with open(path, 'r') as read_obj:
        csv_reader = reader(read_obj)
    ...
    numVariableNodes = int(lines[4])   # Vream and Vimag
    ...
    if len(covariance_rows[0]) != numMeasurements:
        print("ERROR: len(variance_rows[0]) != numMeasurements")
        return
